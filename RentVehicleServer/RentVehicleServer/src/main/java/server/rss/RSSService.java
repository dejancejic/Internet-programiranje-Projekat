...
public class RSSService {
	...
	public String getRssFeed() { 
	    ...
	    for (Post post : posts) {
	        ...
	        rssFeed.append("<link>https://localhost:8443/post?id=").append(post.getId()).append("</link>\n");
	    ...
	    for (Promotion promotion : promotions) {
	        ...
	        rssFeed.append("<link>https://localhost:8443/promotions?id=").append(promotion.getId()).append("</link>\n");
	        rssFeed.append("<description>").append(escapeXml(promotion.getDescription())).append("</description>\n");

	        if (promotion.getDate() != null) {
	            String pubDate = promotion.getDate()
	                .atStartOfDay(ZoneId.systemDefault())
	                .format(formatter);
	            rssFeed.append("<pubDate>").append(pubDate).append("</pubDate>\n");
	        }
